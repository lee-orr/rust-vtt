...
use super::map_zones::{
    DirtyZone, GetDistanceField, ShapeOperation, Zone, ZoneBoundary, ZoneBounds, ZoneBrush,
    ZoneColor, ZoneHierarchy, ZoneOrderingId, ZoneShape, ZoneBrushes,
};
...
fn mesh_tiles(
    ...
    let tile_radius = tile_settings.tile_size / 2.;
    for (entity, tile, position, contents) in tiles_to_update.iter() {
        ...
        if let Some(zone) = tile.zones.first() {
            if let Some(zone) = hierarchy.zone_by_order_id.get(zone) {
                if let Ok((_, zone, color, boundary)) = zones.get(*zone) {
                    let color = if let Some(color) = color {
                        color.color
                    } else {
                        Color::rgb(0.5, 0.5, 0.9)
                    };
                    let content = commands.spawn_bundle(PbrBundle {
                        mesh: meshes.add(shape::Box::new(tile_settings.tile_size, 0.1, tile_settings.tile_size).into()),
                        material: materials.add(color.into()),
                        transform: Transform::from_translation(Vec3::ZERO),
                        ..Default::default()
                    }).id();
                    commands.entity(entity).add_child(content).insert(TileContents { contents: vec![content]});
                }
            }
        }
    }
