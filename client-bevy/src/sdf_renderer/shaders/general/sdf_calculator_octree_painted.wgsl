...
fn objectSceneSDF(point: vec3<f32>, current_epsilon: f32, stack: ptr<function, array<NodeStackItem, MAX_BRUSH_DEPTH>>) -> vec4<f32> {
    var color : vec4<f32> = (99999., 0., 0., 0.);
    for (var i : i32 = 0; i < num_objects.num_objects; i = i + 1) {
        var brush_dist = processNodeColor(point, i, current_epsilon, vec3<f32>(0., 0., 0.), stack);
        if (brush_dist.x < dist.x) {
            dist = brush_dist
        }
    }
    return dist;
...
fn sceneColor(point: vec3<f32>,  current_epsilon: f32, stack: ptr<function, array<NodeStackItem, MAX_BRUSH_DEPTH>>) -> vec4<f32> {
