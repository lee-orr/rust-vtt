...
use std::{net::{Ipv4Addr, SocketAddr, SocketAddrV4}, path::PathBuf, str::FromStr};
use warp::Filter;
use sled::{self, Db};
...
fn parse_arguments () -> (SocketAddr, PathBuf) {
    ...
    let current_directory = String::from(document_dir().unwrap_or_default().to_str().unwrap_or_default());
...
async fn main() {
    ...
    let dbResult = setup_database(&directory);
    
    if let Ok(_) = dbResult {
